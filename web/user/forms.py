...
class SignUpForm(forms.Form):   
    username = forms.CharField(max_length = 11, label = '아이디')
    ...
    strategy = forms.ChoiceField(choices=strategyType, label = '투자 전략 타입')
...
class MyPageForm(forms.Form):
    ...
    def clean(self):
        ...
        email = cleaned_data.get('email')
        strategy = cleaned_data.get('strategy')
        if password1 and password2 and email and strategy:
            if check_password(password1, make_password(password2)):
                return 
            else:
                self.add_error('password2', '비밀번호 확인이 비밀번호와 다름')